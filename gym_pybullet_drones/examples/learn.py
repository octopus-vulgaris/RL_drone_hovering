...
def run(output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=False, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO):
    ...
    #             device="cuda"
    #             )
    # model = PPO.load("ppo_hover-aviary", env=env)
    model = PPO.load("ppo_hover-aviary-cuda", env=env, device="cuda")
    
    policy = model.policy
    input_shape = (1,) + model.observation_space.shape
    summary(policy, input_size=input_shape)

    # model.learn(total_timesteps=1e6, progress_bar=True, callback=eval_callback) # Typically not enough

    # model.save("ppo_hover-aviary-cuda")
    # del model

    # model = PPO.load("ppo_hover-aviary-cuda", env=env)

    #### Plot results
    # plot_results([log_dir], 1e5, results_plotter.X_TIMESTEPS, "hover-aviary")
    # plt.show()
